...
- Sem mapa incorporado: a localização é um botão que abre o Perfil da Empresa no Google (`googleProfileUrl`), com endereço, fotos e avaliações. Isso tira ~600 KB e o último iframe de terceiros da página — a CSP pôde fechar em `frame-src 'none'`. O mesmo link entra no JSON-LD (`hasMap` e `sameAs`) para o Google casar o site com a ficha do negócio.
